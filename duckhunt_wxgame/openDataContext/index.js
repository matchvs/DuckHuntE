...
const assetsUrl = {
  ...
  back: "openDataContext/assets/btn-back.png"
};
...
function drawRankPanel() {
  ...
  //根据title的宽高计算一下位置;
  const titleX = offsetX_rankToBorder + (rankWidth - title.width * 2) / 2;
  const titleY = offsetY_rankToBorder + title.height * 4 / 5;
  const titleWith = title.width * 2;
  const titleHeight = title.height + 100;
...
function init() {
  ...
  preOffsetY = (rankHeight - barHeight * 0.8) / (perPageMaxNum + 1) ;
  fontSize = Math.floor(stageWidth / 20);
  startX = offsetX_rankToBorder + (rankWidth - barWidth * 0.8) / 2;
 ...
 */
function drawButton() {
  // context_drawImage(assets.back, backButtonX, backButtonY, assets.back.width * 2, assets.back.height *2);
  //设置字体
  context.font = fontSize * 0.5 + "px Arial";
  //绘制序号
  context_drawImage(assets.button, nextButtonX , nextButtonY + 120, buttonWidth * 0.5, buttonHeight * 0.5);
  context.fillText("下一页", nextButtonX + 100, nextButtonY + 210, textMaxSize);
  context_drawImage(assets.button, lastButtonX + 300, lastButtonY + 120, buttonWidth * 0.5, buttonHeight * 0.5);
  context.fillText("上一页", lastButtonX + 400, lastButtonY + 210, textMaxSize);
}
...
function drawByData(data, i) {
  let x = startX;
  //绘制底框
  context_drawImage(assets.box, startX, startY + i * preOffsetY + 120, barWidth * 0.8, barHeight * 0.3);
  x += 60;
  //设置字体
  context.font = fontSize * 0.5 + "px Arial";
  //绘制序号
  context.fillText(data.key + "", x, startY + i * preOffsetY + textOffsetY - 100, textMaxSize);
  x += indexWidth + intervalX;
  //绘制头像
  let image = wx.createImage();
  let iconURL = data["url"];
  image.src = iconURL;
  image.width = avatarSize * 0.2;
  image.height = avatarSize *0.2;
  image.onload = function () {
    context_drawImage(image, x, startY + i * preOffsetY + (barHeight - avatarSize *0.2) / 2 -60, avatarSize *0.2, avatarSize * 0.2);
    x += avatarSize + intervalX;
    //绘制名称
    context.fillText(data.name + "", x, startY + i * preOffsetY + textOffsetY - 100, textMaxSize);
    x += textMaxSize + intervalX;
    //绘制分数
    context.fillText(data.scroes + "", x, startY + i * preOffsetY + textOffsetY - 100, textMaxSize);
  };
}
      ...
      for (let i = 0; i < userGameData.length; i++) {
        ...
        for (let j = 0; j < KVDataList.length; j++) {
          if (KVDataList[j].key == 'playRound') {
            ...
            group["name"] = nickname;
            group["url"] = avatarUrl;
            group["scroes"] = parseInt(KVDataList
            [j].value);
      ...
      if (res.errMsg == "getUserCloudStorage:ok") {
        ...
        if (klist.length == 0) {
          ...
          for (let j = 0; j < klist.length; j++) {
            let obj = klist[j];
            if (obj.key == 'playRound') {
              let playRound = parseInt(obj.value) + 1;
              saveMyScore(playRound);
              break;
            }
          }
 ...
 * 保存玩家玩的局数
 */
function saveMyScore(value) {
  wx.setUserCloudStorage({
    KVDataList: [{ 'key': 'playRound', 'value': ('' + value) }],
    success: res => {
      console.log("设置排行榜成功: " + res);
    },
    fail: res => {
      console.log("设置排行榜失败: " + res);
    }
  });
}